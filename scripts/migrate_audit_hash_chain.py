...
def main():
    ...
    try:
        ...
        trigger_rows = conn.execute(
            ...
            "WHERE type='trigger' AND tbl_name='audit_logs' AND sql IS NOT NULL"
        ).fetchall()
        trigger_definitions = [(str(row["name"]), str(row["sql"])) for row in trigger_rows]
        for trigger_name, _ in trigger_definitions:
        ...
        for _, trigger_sql in trigger_definitions:
