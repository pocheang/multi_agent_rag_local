...
_REQUEST_RETRY_BUDGET: ContextVar[RetryBudget | None] = ContextVar("request_retry_budget", default=None)


@contextmanager
def retry_budget_scope(budget: RetryBudget):
    """Install one request-owned budget for every nested retry call."""
    token = _REQUEST_RETRY_BUDGET.set(budget)
    try:
        yield
    finally:
        _REQUEST_RETRY_BUDGET.reset(token)
...
def call_with_retry(op_name: str, fn: Callable[[], Any]) -> Any:
    ...
    for i in range(1, attempts + 1):
        if budget is not None:
            ...
            _REQUEST_RETRY_BUDGET.set(budget)
        try:
            return fn()
