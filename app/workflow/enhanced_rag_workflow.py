"""Retained strict-profile workflow selected only by orchestration.
...
from app.agents.router.routing import decide_route, RouteDecision
from app.agents.router.validator import validate_route_decision
from app.agents.rag.retrieval_quality import evaluate_retrieval_quality
...
from app.orchestration.degradation_strategies import (
...
class EnhancedRAGWorkflow:
    ...
    def __init__(
        ...
        Args:
            ...
            max_answer_retries: Maximum answer regeneration retry attempts. The strict-quality budget caps this at one (initial + one retry).
        ...
        self.max_answer_retries = min(1, max(0, max_answer_retries))
        ...
        logger.info(
            ...
            f"answer_retries={self.max_answer_retries}, "
    ...
    async def _execute_query_internal(
        ...
        tracker = get_tracker()
        execution_id = tracker.start_execution(query=query, user_id=user_id, profile="strict_quality")
        ...
        try:
            ...
            if self.enable_context_tracking:
                context_hints = get_context_aware_routing_hints(session_id, query, user_id=user_id)
            ...
            execution_metadata["validation_trigger_reasons"].append(f"route:{route_validation.validation_reason}")
            ...
            execution_metadata["validation_trigger_reasons"].append(f"answer:{answer_validation.validation_method}")
            ...
            return {
                ...
                "execution_metadata": {
                    ...
                    "profile": execution_metadata["profile"],
                    "model_call_count": execution_metadata["model_call_count"],
                    "input_tokens": execution_metadata["input_tokens"],
                    "output_tokens": execution_metadata["output_tokens"],
                    "token_usage_available": execution_metadata["token_usage_available"],
                    "validation_trigger_reasons": execution_metadata["validation_trigger_reasons"],
                    "regeneration_count": execution_metadata["regeneration_count"],
    ...
    async def _generate_answer_with_validation(
        ...
        while answer_retry_count <= self.max_answer_retries:
            ...
            execution_metadata["regeneration_count"] += 1
            # Exponential backoff: 100ms for first retry, 500ms for second
