...
class RouterConfig(BaseModel):
    ...
    low_confidence_threshold: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Threshold for triggering fallback with reasoning model"
    )
...
class VectorRAGConfig(BaseModel):
    ...
    enable_evaluation: bool = Field(
        default=False,
        description="Enable retrieval quality evaluation"
    )
    retrieval_strategy: str = Field(
        default="hybrid",
        description="Retrieval strategy: hybrid, dense, bm25, rerank"
    )
    dynamic_parameters: bool = Field(
        default=True,
        description="Enable dynamic parameter tuning based on query complexity"
    )
...
class GraphRAGConfig(BaseModel):
    ...
    enabled: bool = Field(
        default=True,
        description="Enable graph RAG"
    )
    min_quality: float = Field(
        default=0.3,
        ge=0.0,
        le=1.0,
        description="Minimum PDF quality threshold for graph queries"
    )
    enable_pdf_optimization: bool = Field(
        default=True,
        description="Enable PDF-aware optimization"
    )
    enable_enhancements: bool = Field(
        default=True,
        description="Enable enhanced features"
    )
    fallback_to_vector: bool = Field(
        default=True,
        description="Fallback to vector RAG on errors"
    )
...
class ReActConfig(BaseModel):
    ...
    use_reasoning: bool = Field(
        default=False,
        description="Use reasoning model for better quality"
    )
    enable_tool_cache: bool = Field(
        default=True,
        description="Cache tool results within session"
    )
...
class SynthesisConfig(BaseModel):
    ...
    use_reasoning: bool = Field(
        default=False,
        description="Use reasoning model for synthesis"
    )
    enable_fact_verification: bool = Field(
        default=True,
        description="Enable fact verification"
    )
    ...
    force_language: Optional[str] = Field(
        default=None,
        description="Force output language (zh/en)"
    )
...
class QualityConfig(BaseModel):
    ...
    enable_route_validation: bool = Field(
        default=True,
        description="Enable route validation"
    )
    enable_retrieval_quality: bool = Field(
        default=True,
        description="Enable retrieval quality assessment"
    )
    enable_answer_validation: bool = Field(
        default=True,
        description="Enable answer validation"
    )
    max_route_retries: int = Field(
        default=1,
        ge=0,
        le=3,
        description="Maximum route retries on low confidence"
    )
    max_answer_retries: int = Field(
        default=1,
        ge=0,
        le=3,
        description="Maximum answer retries on validation failure"
    )
    high_quality_threshold: float = Field(
        default=0.85,
        ge=0.0,
        le=1.0,
        description="Threshold for high quality classification"
    )
    medium_quality_threshold: float = Field(
        default=0.7,
        ge=0.0,
        le=1.0,
        description="Threshold for medium quality classification"
    )
...
class UnifiedAgentConfig(BaseModel):
    ...
    This replaces scattered configuration files:
    - agent_config.py
    - router_config.py
    - graph_rag_config.py
    - quality_config.py
    ...
    timeout_seconds: int = Field(
        ...
        ge=1,
        le=300,
        description="Global timeout for agent execution"
    )
