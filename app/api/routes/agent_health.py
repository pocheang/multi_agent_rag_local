...
async def check_agent_health(agent_name: str) -> dict[str, Any]:
    ...
    **Example:**
    ...
    """
    validator_methods = {
        "router": AgentValidator.validate_router_agent,
        "vector_rag": AgentValidator.validate_vector_rag_agent,
        "graph_rag": AgentValidator.validate_graph_rag_agent,
        "react": AgentValidator.validate_react_agent,
        "synthesis": AgentValidator.validate_synthesis_agent,
        "enhanced_router": AgentValidator.validate_enhanced_router_agent,
        "workflow": AgentValidator.validate_workflow,
    }

    if agent_name not in validator_methods:
        raise HTTPException(
            status_code=404, detail=f"Agent '{agent_name}' not found. Available: {list(validator_methods.keys())}"
        )

    try:
        result = validator_methods[agent_name]()
        return result
    except Exception as e:
    ...
    **Response:**
    ...
    try:
        ...
        return {
            "routes": VALID_ROUTES,
            "skills": VALID_SKILLS,
            "agent_classes": VALID_AGENT_CLASSES,
            "retrieval_strategies": ["hybrid", "dense", "bm25", "rerank"],
            "configuration": {
