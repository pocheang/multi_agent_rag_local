...
runtime_metrics.inc_agent_execution(agent_name="EnhancedRouterAgent", status="success", route="vector")

# 检索质量分数（按策略）
runtime_metrics.observe_retrieval_quality(score=0.92, strategy="hybrid")

# LLM API成本追踪（按提供商/模型）
runtime_metrics.inc_llm_cost(cost_usd=0.0012, provider="openai", model="gpt-4")

# 缓存命中率统计（按层级）
runtime_metrics.inc_cache_operations(operation="retrieval", hit=True, layer="l1")

# 用户会话时长（按用户类型）
runtime_metrics.observe_session_duration(duration_seconds=1800, user_type="premium")
```
...
1. **[监控审计报告](../archive/legacy/docs-root/code_monitoring_management_audit.md)** (15页)
   - 完整问题分析（15个问题）
   - 优先级矩阵
   - 修复建议

2. **[监控指标使用指南](../operations/monitoring/metrics.md)** (12页)
   - 业务指标集成
   - Prometheus查询示例
   - Grafana仪表盘配置
   - 告警规则示例

3. **[日志迁移指南](../operations/logging-migration.md)** (10页)
   - Structlog配置
   - 迁移步骤
   - 5种迁移模式
   - ELK Stack集成

4. **[监控部署指南](../operations/monitoring/deployment.md)** (15页)
   - Docker Compose配置
   - 生产环境配置
   - 故障排查
   - 最佳实践

5. **[监控实施路线图](../archive/legacy/docs-root/monitoring_implementation_roadmap.md)** (12页)
   - 5个阶段计划
   - 时间线和里程碑
   - 资源需求
   - 风险缓解

6. **[快速参考](../reference/agents/quick-reference.md)** (8页)
   - 常用命令
   - API端点
   - Prometheus查询
   - 调试技巧

7. **[MONITORING_README](../operations/monitoring/README.md)** (10页)
   - 功能概览
   - 快速开始
   - 完整文档索引
